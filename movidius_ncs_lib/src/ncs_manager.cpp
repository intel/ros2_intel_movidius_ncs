...
NCSManager::NCSManager(
  const int & max_device_number, const int & start_device_index,
  const Device::LogLevel & log_level, const std::string & cnn_type,
  const std::string & graph_file_path, const std::string & category_file_path,
  const int & network_dimension, const std::vector<float> & mean,
  const float & scale, const int & top_n)
...
void NCSManager::initDeviceManager()
{
  ...
  while (mvncGetDeviceName(device_count_, device_name, sizeof(device_name)) !=
  ...
  for (int device_index = start_device_index_; device_index < device_count_;
    device_index++)
  {
    auto ncs_handle = std::make_shared<movidius_ncs_lib::NCS>(
      device_index, static_cast<Device::LogLevel>(log_level_), cnn_type_,
      graph_file_path_, category_file_path_, network_dimension_, mean_,
      scale_, top_n_);
    ncs_handle_list_.push_back(ncs_handle);
  }
}

void detach(std::thread & t) {t.detach();}
  ...
  while (waiting_time <= WAITING_THRESHOLD) {
    if (!image_list_.empty()) {
      ...
      if (!cnn_type_.compare("alexnet") || !cnn_type_.compare("googlenet") ||
        ...
        !cnn_type_.compare("inception_v2") ||
        !cnn_type_.compare("inception_v3") ||
        !cnn_type_.compare("inception_v4") ||
        !cnn_type_.compare("mobilenet") || !cnn_type_.compare("squeezenet"))
      {
        ...
        ClassificationResultPtr result =
          ncs_handle_list_[device_index]->getClassificationResult();

        p_c_(result, first_image_header);
      } else {
        ncs_handle_list_[device_index]->loadTensor(first_image);
        ncs_handle_list_[device_index]->detect();
        DetectionResultPtr result =
          ncs_handle_list_[device_index]->getDetectionResult();
...
void NCSManager::startThreads()
{
  ...
  // RCLCPP_INFO(this->get_logger(), "started %d threads", device_count_ -
  // start_device_index_);
}

std::vector<ClassificationResultPtr> NCSManager::classifyImage(
...
#pragma omp parallel for
    for (int device_index = 0;
      device_index < device_count_ - start_device_index_; device_index++)
    {
      cv::Mat imageData = cv::imread(
        images[i * (device_count_ - start_device_index_) + device_index]);
  ...
  for (int j = 0; j < parallel_left; j++) {
    cv::Mat imageData = cv::imread(
      images[parallel_group * (device_count_ - start_device_index_) + j]);
    ncs_handle_list_[j]->loadTensor(imageData);
    ncs_handle_list_[j]->classify();
    ClassificationResultPtr result =
      ncs_handle_list_[j]->getClassificationResult();
...
std::vector<DetectionResultPtr> NCSManager::detectImage(
  const std::vector<std::string> & images)
{
  int image_size = static_cast<int>(images.size());
...
#pragma omp parallel for
      ...
      device_index < device_count_ - start_device_index_; device_index++)
    {
      ...
      DetectionResultPtr result =
        ncs_handle_list_[device_index]->getDetectionResult();
      results[i * (device_count_ - start_device_index_) + device_index] =
        std::make_shared<DetectionResult>(*result);
    }
  }

  for (int i = 0; i < parallel_left; i++) {
    cv::Mat imageData = cv::imread(
      images[parallel_group * (device_count_ - start_device_index_) + i]);
    ...
    results[parallel_group * (device_count_ - start_device_index_) + i] =
      std::make_shared<DetectionResult>(*result);
  }
...
void NCSManager::classifyStream(
  const cv::Mat & image, FUNP_C cbGetClassificationResult,
