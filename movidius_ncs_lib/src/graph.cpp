...
{
Graph::Graph(const std::shared_ptr<Device>& device, const std::string& graph_buf,
             int network_dimension)
  : graph_buf_(graph_buf), network_dimension_(network_dimension), handle_(nullptr)
{
...
std::string Graph::getDebugInfo()
{
  ...
  unsigned int length;
  int ret =
      mvncGetGraphOption(handle_, MVNC_DEBUG_INFO, reinterpret_cast<void**>(&debug_info), &length);
...
float Graph::getTimeTaken()
{
  ...
  unsigned int length;
  int ret =
      mvncGetGraphOption(handle_, MVNC_TIME_TAKEN, reinterpret_cast<void**>(&time_taken), &length);
  ExceptionUtil::tryToThrowMvncException(ret);
...
{
  int ret = mvncAllocateGraph(device_handle, &handle_, graph_buf_.c_str(), graph_buf_.size());
  ExceptionUtil::tryToThrowMvncException(ret);
}
