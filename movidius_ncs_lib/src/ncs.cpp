...
{
NCS::NCS(int device_index, Device::LogLevel log_level, const std::string& cnn_type,
...
void NCS::classify()
{
  ...
  try
  {
    ...
    unsigned int length;
    int ret = mvncGetResult(graph_->getHandle(), reinterpret_cast<void**>(&probabilities), &length,
                            &user_param_);
    ...
    auto cmp = [](const Item& a, const Item& b) { return a.probability > b.probability; };
...
void NCS::detect()
{
  ...
  try
  {
    uint16_t* result;
    unsigned int length;
    int ret = mvncGetResult(graph_->getHandle(), reinterpret_cast<void**>(&result), &length,
                            &user_param_);
    ExceptionUtil::tryToThrowMvncException(ret);
...
void NCS::loadTensor(const cv::Mat& image)
{
  ...
  assert(graph_->getHandle() != nullptr);
  int ret = mvncLoadTensor(graph_->getHandle(), tensor_->raw(), tensor_->size(), user_param_);
  ExceptionUtil::tryToThrowMvncException(ret);
}
...
void NCS::initDevice()
{
  device_.reset(new Device(device_index_, static_cast<Device::LogLevel>(log_level_)));
}
...
void NCS::splitIntoLines(const std::string& content, std::vector<std::string>& lines)
{
