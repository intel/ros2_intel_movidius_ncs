...
void Result::parseYoloResult(
  const std::vector<float> & result,
  const std::vector<std::string> categories, int img_width,
  int img_height)
{
  ...
  int img_height)
{
  ...
  for (int i = 0; i < num_detection; i++) {
    ...
    ItemInBBox obj_in_bbox;
    if (std::isnan(category_id) || std::isnan(probability) || std::isnan(xmin) || xmin < 0 ||
      xmin > img_width || std::isnan(ymin) || ymin < 0 || ymin > img_height || std::isnan(xmax) ||
      xmax < 0 || xmax > img_width || std::isnan(ymax) || ymax < 0 || ymax > img_height)
    {
      continue;
    } else {
...
float Result::IOU(ItemInBBox box1, ItemInBBox box2)
{
  ...
  return inter_area * 1.0 /
         (box1.bbox.width * box1.bbox.height + box2.bbox.width * box2.bbox.height - inter_area);
}
}  // namespace movidius_ncs_lib
