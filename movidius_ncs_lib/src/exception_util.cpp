...
std::map<int, std::string> CODE2STR = {
  {MVNC_BUSY, "device is busy, retry later"},
  {MVNC_ERROR, "an unexpected error was encontered during the function call"},
  {MVNC_OUT_OF_MEMORY, "the host is out of memory"},
  {MVNC_DEVICE_NOT_FOUND, "there is no device at the given index or name"},
  {MVNC_INVALID_PARAMETERS, "at least one of the given parameters is invalid in the context of "
    "the function call"},
  {MVNC_TIMEOUT, "timeout in the communication with the device"},
  {MVNC_MVCMD_NOT_FOUND, "the file named MvNCAPI.mvcmd should be installed in the mvnc "
    "direcotry"},
  {MVNC_NO_DATA, "no data to return"},
  {MVNC_GONE, "the graph or device has been closed during the operation"},
  {MVNC_UNSUPPORTED_GRAPH_FILE, "the graph file may have been created with an incompatible prior "
    "version of the Toolkit"},
  {MVNC_MYRIAD_ERROR, "an error has been reported by Movidius VPU"}
};
